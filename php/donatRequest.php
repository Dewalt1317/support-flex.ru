...
if (date("H:i:s") >= $TempDonat->timeRequest) {
    $dataBD = [];
            ...
            foreach ($data['data'] as $donation) {
                ...
                foreach ($dataBD as $row) {
                    if ($row['id'] == $donationID) {
                        $idExists = true;
                        // Если поле `messageID` пустое, устанавливаем $messageSend в false
                ...
                if (!$donatioMessageSend) {
                    messageSend($donorName, $botStart, $amount, $donatioMessage, $botEnd, $idExists, $donationID, $mysql);
                }
                // Если id не существует, выполняем запись в базу данных
                if (!$idExists) {
                    $messageID = messageSend($donorName, $botStart, $amount, $donatioMessage, $botEnd, $idExists, $donationID, $mysql);
